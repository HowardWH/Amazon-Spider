...
func InsertDetailMysql(item map[string]string) error {
	sql := "REPLACE INTO `Asin" + Today + "`(id,bigname,title,rank,createtime,price,reviews,ship,sold,score,img,relate) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"
	// Todo
	...
	_, err := DataDb.Insert(sql, item["id"], item["bigname"], item["title"], item["rank"], item["createtime"], pricetemp, reviews, item["ship"], item["sold"], score, img, item["relate"])
	// Todo
	tablename := hashcode(item["id"])
	AmazonAsinLog.Debugf("insert A %s: %s", tablename, item["id"])
	hahasql := "REPLACE INTO `A" + tablename + "`(id,day,bigname,title,rank,createtime,price,reviews,ship,sold,score,img,relate) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
	_, errr := HashDb.Insert(hahasql, item["id"], Today, item["bigname"], item["title"], item["rank"], item["createtime"], pricetemp, reviews, item["ship"], item["sold"], score, img, item["relate"])
	if err == nil {
		return errr
	}
	return err
